// server.js - Universal OpenAI to NVIDIA NIM Proxy (Smart Tag / Prompt Override)
...
app.post('/v1/chat/completions', async (req, res) => {
  ...
  try {
    ...
    // Explicitly command the model to output the literal <think> string itself
    const FORCE_THINKING_PROMPT = "\n\n[CRITICAL DIRECTIVE: You MUST think step-by-step before answering. You MUST start your response exactly with the literal string \"<think>\". Write your internal roleplay planning, then output \"</think>\", and only then write your dialogue. Do not skip this.]";
              ...
              if (data.choices?.[0]?.delta) {
                ...
                if (SHOW_REASONING) {
                  ...
                  if (reasoning) {
                    ...
                    // Strip native tags from reasoning stream if the model accidentally double-tags it
                    reasoning = reasoning.replace(/<think>/gi, '').replace(/<\/think>/gi, '');
                    streamText += reasoning;
                  }

                  if (content) {
                    // If we just swapped from reasoning channel to content channel, close the tag
                    if (channelReasoningActive) {
                      streamText += '\n</think>\n\n';
                      channelReasoningActive = false;
                    }

                    // Normalize Moonshot tags to Janitor format
                    content = content.replace(/<thought>/gi, '<think>').replace(/<\/thought>/gi, '</think>');

                    // Prevent `<think></think><think>` if both NIM channel and the Model prompt output a tag
